...
if __name__ == "__main__":
    ...
    path = os.path.join("response", f"{args.run_name_red}.json")
    generations_red = json.load(open(path, "r"))

    path = os.path.join("response", f"{args.run_name_blue}.json")
    ...
    for red, blue in tqdm(zip(generations_red, generations_blue), total=len(generations_red)):
        prompt = red["prompt"]
        response_red = clean(clean(red["response"][len(prompt) :], "###USER:"), "\n\nUSER:")
        response_blue = clean(clean(blue["response"][len(prompt) :], "###USER:"), "\n\nUSER:")
    ...
    if not os.path.exists("gpt-4-evaluations"):
        os.makedirs("gpt-4-evaluations")
